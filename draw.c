...
draw_create_pixmap(Draw *ctx) {
    ctx->drawable = XCreatePixmap(ctx->dpy, ctx->root, ctx->w, ctx->h,
                                   ctx->depth);
...
static void
draw_resize(Draw *ctx, uint32 w, uint32 h) {
    if (ctx == NULL) {
        return;
    }
...
draw_picture_create_resized(Draw *ctx,
    ...
    if ((src_w <= dst_w*xrender_scale_limit)
        ...
        pm = XCreatePixmap(ctx->dpy, ctx->root, src_w, src_h, argb_depth);
        gc = XCreateGC(ctx->dpy, pm, 0, NULL);
        XPutImage(ctx->dpy, pm, gc, &img, 0, 0, 0, 0, src_w, src_h);
        XFreeGC(ctx->dpy, gc);
...

    {
        ...
        if ((origin = imlib_create_image_using_copied_data((int32)src_w,
                                                           (int32)src_h,
                                                           src)) == NULL) {
            return None;
        }
...

        {
            ...
            pm = XCreatePixmap(ctx->dpy, ctx->root, dst_w, dst_h,
                               argb_depth);
            gc = XCreateGC(ctx->dpy, pm, 0, NULL);
            XPutImage(ctx->dpy, pm, gc, &img, 0, 0, 0, 0, dst_w, dst_h);
            imlib_free_image_and_decache();
            XFreeGC(ctx->dpy, gc);
...
draw_rect(Draw *ctx,
    ...
    if (filled) {
        XFillRectangle(ctx->dpy, ctx->drawable, ctx->gc, x, y, w, h);
    } else {
        XDrawRectangle(ctx->dpy, ctx->drawable, ctx->gc, x, y, w - 1,
                       h - 1);
    }
    return;
}
...
draw_text(Draw *ctx,
    ...
    while (*text) {
        ...
        for (curfont = ctx->fonts; curfont; curfont = curfont->next) {
            if (XftCharExists(ctx->dpy, curfont->xfont,
                              (uint32)utf8codepoint)) {
                usedfont = curfont;
                break;
            }
            ...
            while (*scan) {
                ...
                if (is_combo) {
                    ...
                    for (f = ctx->fonts; f; f = f->next) {
                        if (XftCharExists(ctx->dpy, f->xfont, (uint32)cp)) {
                            break;
                        }
                    }
                }
...
static void
draw_map(Draw *ctx, Window win, int32 x, int32 y, uint32 w, uint32 h) {
    if (ctx == NULL) {
        return;
    }

    XCopyArea(ctx->dpy, ctx->drawable, win, ctx->gc, x, y, w, h, x, y);
    XSync(ctx->dpy, False);
    return;
}
...
draw_fontset_getwidth(Draw *ctx, char *text) {
    ...
    return (uint32)draw_text(ctx, 0, 0, 0, 0, 0, text, 0);
}

static uint32
draw_fontset_getwidth_clamp(Draw *ctx, char *text, uint32 n) {
    uint32 tmp = 0;

    if (ctx && ctx->fonts && text && (n > 0)) {
        tmp = (uint32)draw_text(ctx, 0, 0, 0, 0, 0, text, (int32)n);
    }
